...
description: Embed Mermaid diagrams inside code fences in Markdown. No clien-side JavaScript. Dark mode support. SVG
---
test
```mermaid
---
...
- `@beoe/rehype-mermaid` - my experiment. It is based on `mermaid-isomorphic` (the same one used by `rehype-mermaid`). But it also adds:
  - [Selector based dark mode](https://github.com/remcohaszing/rehype-mermaid/issues/16). Used by Starlight and others
  - cache - to prevent permanent launch of headless browser
  - out of the box compatible with [[svg-pan-zoom|pan and zoom "plugin"]]
...
6. **Optional** add [[svg-pan-zoom#installation|pan and zoom for diagrams]]
